...
我们做出**四项可主张的贡献**，两项形式化和两项操作性；
可执行安全引用模式是第一项，其余项是该模式有效的操作性证据。

- **C1 — 第三方可以验证的可重现安全工件。** Ghost 将属性
  谓词（ADR）、运行（内容寻址的 MCAP）和验证器（OIDC 签名的
  PyPI wheel）打包为一个可引用的单元；机械检查的不变式
  （TLA+/TLC）、Hypothesis property tests 和 CI gating 支
  撑验证器本身。被引用的工件*就是*证伪机制 —— 我们将此模式
  称为*可执行的安全引用*。这是我们认为真正具有差异性的模式；
  论文的其余部分是它在实践中有效的证据，包括在真实飞行遥测
  上（§8.7–§8.8）。
