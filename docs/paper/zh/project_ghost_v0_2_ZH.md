...
**论点：安全声明应当通过可重现的引用变得可执行和可证伪。** 今
天它们通常以散文形式陈述，并通过读者无法重新运行的模拟视频来
说明。我们介绍 **Project Ghost**，这是一个开源平台，其主要贡
献是**一个引用模式，允许第三方通过单个 shell 命令对录制的运
行进行 byte-exact 的安全声明验证**：
`pip install project-ghost==0.2.3`，然后
`ghost verify-properties --mcap <log>`。该模式将七个已有要素
—— ADR、内容寻址的 MCAP 遥测、纯函数验证器、Hypothesis
property tests、CI gating、标记发布、OIDC 签名的 PyPI wheels
—— 组合成一个一致的可重现单元，并通过 TLA+/TLC 额外检查底层
不变式。
...
`pip install project-ghost==0.2.3` 重新运行。**安全声明应当与
第三方拒绝它所需的一切一起发布，我们相信这在操作上现在已经
可能。**
...
覆盖了完整的条件行为空间的分区定理。恢复延迟界限 的紧致恢复延迟
界限通过镜像验证器算法的单独 TLA+ 规范进行机械验证。

### 1.1 贡献

**关于自主性系统的安全声明今天可以由其作者断言并在其 venues
中说明，但无法被运行发生时不在场的第三方证伪。** Project
Ghost 弥补了这一空缺：安全声明以内容寻址的自主性日志加上一
个 shell 命令（`ghost verify-properties --mcap <log>`）的形式
交付；任何拥有 wheel 和日志的人都可以重现裁定 —— 或反驳它。
我们将此模式称为**可执行的安全引用**。

我们做出**三项贡献**：

- **C1 —— 可执行的安全引用模式（概念性）。** 安全声明发布
  为内容寻址的运行加纯函数验证器，作为一个可发布单元，第三方
  无需信任生产者即可通过单个命令重新派生。被引用的工件*就是*
  证伪机制。

- **C2 —— 参考实现：Ghost（工件）。** 一个闭环自主性监督器，
  在五个属性上实例化该模式（BAUD-v1、ERUR-v1、MD-v1、RLB-v1、
  FPB-v1），具有有约束力的 ADR、内容寻址的 MCAP 遥测、
  `ghost verify-properties --mcap`、OIDC 签名的 PyPI wheels，
  以及一个策略无关的验证器，跨三个校准策略（§8.4）。
...
恢复延迟界限（§6.3）—— 用于滑动窗口 count-of-K-in-W 滤波器的封闭
...
FDI 要素已被广泛建立。恢复延迟界限是辅助结果，而非贡献。§5.3
的分区定理*在我们机械化的形式上*是新颖的 —— 参考闭环上的 TLA+
`INV_PARTITION`。我们捍卫的贡献是**工程化组合**（C1）及其
**操作演示**（C2）：该装配在我们调查的开放 autonomy-safety
工具中缺失。
...
**Ghost 将安全声明、其运行和其验证器打包为第三方可以重新派生
的可重现单元。** load-bearing 贡献是操作性的，而非理论性的：
ADR、内容寻址的 MCAP、纯函数 CLI 验证器、Hypothesis property
tests、对底层不变式的 TLA+/TLC 机械检查，以及 OIDC 签名的
PyPI wheel —— 组装使得引用*就是*证伪机制。我们将此模式称为
**可执行的安全引用**。

参考自主性监督器上五个属性的参考实例化是证明该模式可行的证据，
而非贡献本身。推动论文的是*装配*使安全声明在散文-视频断言所
不能的方式上具有可操作的可证伪性。工件可从
`pip install project-ghost==0.2.3` 重新运行；贡献的成立或失败
取决于结果判定是否意味着 ADR 所声明的含义。我们相信确实如此。
