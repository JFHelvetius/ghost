...
- **C1 — 可执行安全引用模式。** 七个现有要素的组合 —— ADR
  + 内容寻址的 MCAP + 纯函数验证器 + Hypothesis property
  test + CI gate + 标记发布 + OIDC 签名的 PyPI wheel ——
  组装使得被引用的工件*本身就是证伪机制*。这是我们认为真正
  具有差异性的模式；论文的其余部分是它在实践中有效的证据，
  包括在真实飞行遥测上（§8.7–§8.8）。
