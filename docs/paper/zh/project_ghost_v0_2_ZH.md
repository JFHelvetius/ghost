...
`pip install project-ghost==0.2.2` 重新运行。
...
我们将贡献**按从最 load-bearing（一个工程模式）到最狭窄（一个有
用的支持性界限）的顺序排列**：
...
- **C2 — 具有已证明检测能力的可重现性原语。** 一个对内容寻址
  ...
  runner 之间产生确定性的 JSON 输出（由 CI 强制执行，§8.7），
...
- **C3 — 一个具有机械检查语义的参考自主性监督器的属性集。** 五
  ...
  到端的机械化，而非公式化。
- **C4 — 用于滑动窗口 count-of-K-in-W 滤波器的封闭式紧致恢复
  延迟界限**（Theorem 1，§6.3）：`L ≤ peak + W − 1`，由见证跟
  踪以等式达到（`L = 38 = 7 + 32 − 1`）。该界限直接源于滑动
  窗口机制，事后看来是基本的；我们在文献综述（§2.3）中没有找
  到它以封闭形式陈述，但我们不做更广泛的新颖性声明，将其作为
  C3 的支持证据，而非独立的理论结果。

C1 和 C2 是我们预期将经受时间考验的贡献。C3 在代表性监督器上实
例化它们；C4 提供了 `Rlb.tla` 机械验证的有用界限。我们将这项工
作定位为**系统/工具论文，而非理论论文**。
...
JSON 判定。**本论文的贡献是将七个框组装为一个可发布的单元**；
其他一切（属性集、封闭式界限、TLA+ 规范）在代表性监督器上实例
化该模式。
...
Project Ghost 是**安全声明的引用模式**。其 load-bearing 贡献是这
样的命题：安全声明应当与第三方拒绝它所需的一切一起发布：有约束
力的书面陈述（ADR）、内容寻址的日志（MCAP）、作为 CLI 暴露的纯
函数验证器、property tests、对底层不变式的机械检查（TLA+/TLC），
以及签名的分发渠道（OIDC PyPI）。我们不声称发现了各个组成部分
—— 每个都是标准实践 —— 但我们尚未找到将它们作为可从单个 shell
命令访问的一致单元来交付的工具，我们将该组合视为适当的分析单
元。
