...
calibration policies, exercise it under six categories of injected
bugs (all detected), and run the verifier unchanged on a real PX4
v1.10 flight log. The pattern, not the individual properties, is
the contribution. The artifact is re-runnable from
...
\textbf{Caveat.} The orchestrator uses the ULog's own EKF2 estimate
as both belief and (vacuous) ground truth, so the all-HOLDS row is
vacuous as a safety claim. The \texttt{Sim, not hardware} clause of
\S\ref{sec:limits} remains intact for the strong reading. A
non-vacuous ground truth is candidate ADR-0037.
...
\midrule
Pose samples extracted              & 636 \\
Ghost cycles run                    & 71 \\
MCAP SHA-256                        & \texttt{49fd0a48...720a4591} \\
BAUD/ERUR/MD/RLB/FPB                & all HOLDS (vacuously) \\
\bottomrule
\end{tabular}
...
\textbf{What this establishes}, with the caveat above duly noted,
