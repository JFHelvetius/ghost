...
we ask them to remember. The body of the paper is the operational
evidence for it.
...
hypothesis and mechanises Theorem~\ref{thm:rlb}. \texttt{Fpb.tla}
($\mathrm{MAX\_CYCLES}{=}8$) checks the FPB counter automaton's
structural well-formedness.

\begin{theorem}[Tight recovery latency bound; transient regime]
\label{thm:rlb}
...
$L = \mathrm{peak} + W - 1$, attained with equality. Proof in
Appendix~\ref{app:theorem}.
\end{theorem}

Theorem~\ref{thm:rlb} is presented as an auxiliary result that the
TLA+ spec \texttt{Rlb.tla} mechanises, not as a contribution.
...
\begin{quote}
\itshape
On the same real PX4 v1.10 flight log, the verifier flips its
verdict from all-HOLDS under the reference policies to BAUD-v1
VIOLATED when a one-line decision-policy or actuation-policy
regression is substituted. The verifier is informative on real
telemetry: it discriminates the reference run from two specific
named regressions.
\end{quote}

\textbf{What this does not claim.} The buggy run did not fly
anything; the substitution is at the policy layer. The diversity
of bug categories that flip on real data depends on whether the
ULog's drift pattern exercises their precondition --- generalising
across more flights is scope for ADR-0037 (real-flight corpus).
...
an ADR, an MCAP, a pure-function verifier on a CLI, property
tests, mechanical checks (TLA+/TLC), and a signed PyPI wheel. The
individual ingredients are standard; the assembly, in our
surveyed scope, is missing. The artifact is re-runnable from
\texttt{pip install project-ghost==0.2.3}.
...
\section{Proof of Theorem~\ref{thm:rlb}}
\label{app:theorem}
