...
\textbf{Corpus generalisation.} v0.2.4 also runs the matrix on a
3-ULog PX4 SITL corpus (\texttt{sample.ulg},
\texttt{sample\_appended.ulg}, \texttt{sample\_logging\_tagged.ulg}).
12/18 cells discriminate: the two ULogs with
$\mathit{fire\_fraction} > 0.9$ are fully green (6/6 each); the
third ULog ($\mathit{fire\_fraction} = 0$, largely stationary)
reports HOLDS for 4/6 categories because BAUD-v1's drift
precondition is never exercised. This is informative
non-discrimination, not failure --- the JSON artefact at
\texttt{docs/paper/outputs/multi\_ulog\_discrimination/}
\texttt{matrix.json} carries per-ULog \texttt{fire\_fraction}
diagnostics so readers can distinguish "verifier missed the bug"
from "ULog did not exercise the precondition". The "independent
GT source" mitigation is deferred to ADR-0037 (\S\ref{sec:limits}).
...
\begin{itemize}[noitemsep,topsep=2pt]
  \item \textbf{Sim, not hardware.} v0.2.4 ships the PX4 ULog
        adapter and a 3-ULog SITL corpus; non-PX4 stacks and the
        "independent GT source" mitigation remain under ADR-0037.
