...
\begin{itemize}[noitemsep,topsep=2pt]
  ...
  \item \textbf{Bounded TLC, partial unbounded coverage.}
        v0.2.5 extends TLC's $W = 4$ baseline with a parametric
        sweep at $W \in \{4, 8, 16\}$ (all green, 25/81/289
        distinct states) and a rigorous hand proof of the
        unbounded RLB-v1 theorem (no $W$ dependence in the
        argument). The full TLAPS-mechanical certificate is
        the open follow-up under ADR-0038. The TLAPS outline
        ships per-lemma discharge guidance for a future
        contributor (Linux/macOS install required).
