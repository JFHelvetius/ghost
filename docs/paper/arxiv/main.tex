...
\begin{abstract}
We describe \textbf{Project Ghost}, an open-source reference
implementation of five formal safety properties for autonomy under
uncertainty, verifiable byte-exact from any captured run via a single
shell command: \texttt{ghost verify-properties --mcap <log>}. The
property set (BAUD-v1, ERUR-v1, MD-v1, RLB-v1, FPB-v1) covers four
distinct natures of safety claim --- conditional behaviour in both
drift directions, an unconditional structural property of the
calibrator, a quantitative recovery latency bound, and an empirical
regression-gating observer --- each stated in a binding architectural
decision record (ADR), verified by a pure function over
content-addressed MCAP telemetry, exercised by approximately 50
property tests within a 1665-test suite, witnessed inline in every
reference smoke run, and self-enforced on every push by CI. The
BAUD/ERUR partition and the monotonic-degradation property are
additionally \textbf{mechanically verified} by TLA+/TLC over the
abstract state space of the reference policy pair. We make four
claimable contributions: (1) a closed-form, tight structural recovery
latency bound $L \le \mathrm{peak} + W - 1$ for sliding-window
count-of-$K$-in-$W$ calibration filters (Theorem~\ref{thm:rlb}); (2) a
formal partition theorem $\mathrm{BAUD} \oplus \mathrm{ERUR}$ over the
conditional behaviour space, mechanically verified by TLC; (3) a
reproducibility primitive --- a content-addressed verifier reachable
from \texttt{pip install project-ghost==0.2.0} that detects injected
bugs (\S\ref{sec:violation}) and produces deterministic JSON output
across machines; and (4) an end-to-end safety citation pattern. The
verifier completes in 21\,ms for 10-cycle runs and 406\,ms for
200-cycle runs (linear in trace length), with the property set
holding across all three reference policy parameterisations evaluated.
\end{abstract}

\medskip
\noindent\textbf{Keywords:} verifiable autonomy, formal safety
properties, runtime verification, calibrated confidence, byte-exact
replay verification, TLA+, MCAP, sliding-window monitors.
...
We make \textbf{four claimable contributions}, two formal and two
operational:

\begin{itemize}
  \item \textbf{C1 --- Theorem~\ref{thm:rlb} (tight recovery latency
        bound).} A closed-form upper bound $L \le \mathrm{peak} + W - 1$
        on the recovery latency of any closed-loop autonomy supervisor
        using a sliding window of size $W$ and a count-of-$K$-in-$W$
        threshold over $M$-minimum outcomes, formalised in
        \S\ref{sec:theorem} and shown tight by a witness trace where
        equality is attained ($L = 38 = 7 + 32 - 1$).
  \item \textbf{C2 --- Mechanically verified partition theorem.} The
        conditional behaviour of the reference closed loop partitions
        into $\mathrm{BAUD} \oplus \mathrm{ERUR}$. Stated in TLA+ as
        \texttt{INV\_PARTITION} and verified by TLC over the full
        reachable state space of the abstract model.
  \item \textbf{C3 --- Reproducibility primitive with demonstrated
        ...
        Bug-detection capacity demonstrated in \S\ref{sec:violation}:
        a smoke with an injected buggy calibrator yields
        \texttt{BAUD-v1: VIOLATED}, exit code 1,
        \texttt{violation\_count: 12} on the unmodified verifier.
  \item \textbf{C4 --- End-to-end safety citation pattern.}
        Content-addressed MCAP + ADR + pure-function verifier +
        Hypothesis property test + CI gate + tagged release +
        OIDC-signed PyPI wheel, assembled as a single reproducibility
        unit so a third party can verify any cited safety claim
        against the captured run without trusting the producer.
\end{itemize}

C1 and C2 are formal contributions, novel to the best of our
knowledge after a deliberate prior-art review (\S\ref{sec:related}).
C3 and C4 are operational and pattern contributions; their novelty
rests on the comparison matrix of \S\ref{sec:comparison}.
...
established. C1's mathematics is elementary in hindsight but not, to
our knowledge, previously stated as a closed-form bound in the
peer-reviewed runtime-verification literature. C2's novelty is in
mechanising the partition for sliding-window autonomy supervisors
...
$W{=}4, \mathrm{MAX\_DIRTY\_RUN}{=}12$) mirrors the verifier
algorithm of \texttt{src/project\_ghost/properties/rlb.py} and
checks three invariants: \texttt{INV\_RLB} (the mechanical witness
of Theorem~\ref{thm:rlb}), \texttt{INV\_PEAK\_BOUNDED}, and
\texttt{INV\_WINDOW\_BOUND}.
...
from \texttt{pip install project-ghost==0.2.0}, and additionally
...
\subsection{Bug-detection capability (Violation Showcase)}
\label{sec:violation}

A standalone smoke \texttt{closed\_loop\_smoke\_violated.py}
replaces the reference calibrator with a deliberately broken
passthrough policy. The captured MCAP is fed to the unmodified
verifier:
...
MCAPs. \textbf{Cross-machine byte-equality} is enforced by a CI job
\texttt{determinism-cross-machine}: the reference smoke runs on a
\{\texttt{ubuntu-latest}, \texttt{windows-latest}\} matrix, each
runner publishes the SHA-256 of its MCAP and property-report JSON,
and the aggregator step \texttt{diff}s the two files. Any
...
% =======================================================================
\section{Conclusion}
Project Ghost is not a new theory of autonomy under uncertainty.
It is a \emph{reference of the citation pattern} that the existing
theory deserves: a content-addressed log, a pure-function verifier
on a CLI, a formal property statement in a binding ADR, a
Hypothesis property test, a CI gate, two TLA+ specs mechanically
checked by TLC (including Theorem~\ref{thm:rlb}), a tagged
release, and an OIDC-signed wheel --- all in one artifact, all on
one shell command. The contribution is in the assembly; the
evidence is re-runnable from
\texttt{pip install project-ghost==0.2.0}.
