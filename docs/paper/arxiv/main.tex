...
\node[party, right=14mm of mcap] (cmd) {\texttt{ghost verify-\\properties --mcap}};
...
\toprule
\textbf{Run} & BAUD & ERUR & MD & RLB & FPB & MCAP SHA-256 (prefix) \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Verdict delta on the bundled real PX4 ULog. Both buggy
runs flip BAUD-v1 from HOLDS to VIOLATED on the same physical
flight; the other four properties remain HOLD, so the violation
is isolated to the property the bug attacks.}
\label{tab:realflight-discrim}
\end{table}

\textbf{Reproducibility.} End-to-end runnable from
\texttt{pip install 'project-ghost[adapters]==0.2.3'}:
...
physical flight, swapping the reference decision policy for a
one-line buggy \texttt{PROCEED}-only policy, or the reference
actuator for a one-line unsafe-reason actuator, flips the
verdict. The synthetic violation-matrix detections of \S8.2
transfer to real telemetry, on this ULog, for the bug categories
whose precondition the real flight's drift pattern exercises.
...
\begin{itemize}
  \item \textbf{Sim, not hardware.} MCAPs come from a simulated
        ...
        claims require a HAL backend and a hardware campaign. We
        ship a runnable but unimplemented skeleton of the PX4 ULog
        adapter at \texttt{docs/paper/scripts/}
        \texttt{px4\_ulog\_adapter\_skeleton.py}, with the
        ground-truth-source policy enumerated explicitly
        (motion-capture / RTK GPS / vacuous EKF2 fallback) so that
        a future contributor can close this gap as candidate
        ADR-0037 without re-deriving the integration contract.
...
verifier today (v0.2.3 ships ERUR-v1; v2 is hand-evaluated for
the three calibrators of \S\ref{sec:multi-policy} and lifts to a
generic verifier in v0.2.4 as ADR-0040); that we have a unique
licensing claim on the term (it overlaps with how
epistemic-logic, doxastic-logic and self-assessment communities
have used adjacent vocabulary).
