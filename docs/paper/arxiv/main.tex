...
published feature sets (RTAMT 0.3.5 from PyPI; Ghost v0.2.2). The
...
\begin{tabular}{lcc}
\toprule
\textbf{Capability} & \textbf{Ghost v0.2.2} & \textbf{RTAMT 0.3.5} \\
\midrule
...
MD-v1                                  & HOLDS \\
RLB-v1                                 & HOLDS \\
FPB-v1                                 & HOLDS (fire\_fraction = 0.9437) \\
\bottomrule
\end{tabular}
...
\textbf{Caveat on the verdict.} The orchestrator uses the ULog's
own EKF2 estimate as both the agent's belief and the (vacuous)
oracle ground truth, so the all-HOLDS row is vacuous as a safety
claim. A non-vacuous ground truth (motion capture, RTK GPS,
post-flight optimised solution) is candidate ADR-0037; the
\texttt{Sim, not hardware} clause of \S9 remains intact for the
strong reading.

\textbf{What this section establishes}, with the caveat above duly
