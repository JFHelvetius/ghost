...
exercises. The "independent GT source" mitigation is deferred
to ADR-0037 / v0.2.5.
...
"$\le$ 2/6 detections" sanity check, and the JSON artefact
schema.
...
\begin{itemize}
  ...
  \item \textbf{Vacuous holds on stationary ULogs.}
        \S\ref{sec:multi-ulog-corpus} reports honestly that on
        the corpus's stationary ULog ($\mathit{fire\_fraction} =
        0$), four of the six buggy categories produce HOLDS
        across nominal and buggy because BAUD-v1's drift
        precondition is never exercised. The verifier is correct
        ("nothing fired, nothing to violate") but the row is
        uninformative as a discrimination test. The "independent
        GT source" mitigation under ADR-0037 closes this; we
        defer it to v0.2.5.
\end{itemize}
