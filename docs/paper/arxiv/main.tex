...
\begin{itemize}
  \item \textbf{C1 --- The executable-safety-citation pattern.} A
        composition of seven existing ingredients --- ADR +
        content-addressed MCAP + pure-function verifier + Hypothesis
        property test + CI gate + tagged release + OIDC-signed PyPI
        wheel --- assembled so that the artefact cited \emph{is
        itself the falsification mechanism}. The pattern is what we
        believe is genuinely differentiating; the rest of the paper
        is the evidence that it works in practice, including on
...
smaller claim --- \emph{a third party can verify a stated property
against a captured run by issuing one shell command} --- but it makes
that claim \emph{operationally}, not by appeal to internal review.
...
% =======================================================================
\section{Conclusion}
\textbf{Ghost turns safety claims into executable citations.} That
