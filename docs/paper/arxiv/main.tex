...
\begin{abstract}
\textbf{Thesis: safety claims should be executable and
falsifiable through a reproducible citation.} Today they are
typically asserted in prose and illustrated by simulation videos
the reader cannot re-run. We describe \textbf{Project Ghost}, an
open-source platform whose primary contribution is \textbf{a
citation pattern that lets a third party verify any safety claim
against the recorded run, byte-exact, via a single shell command}:
\texttt{pip install project-ghost==0.2.3}, then
\texttt{ghost verify-properties --mcap <log>}. The pattern composes
seven existing ingredients --- ADRs, content-addressed MCAP
telemetry, pure-function verifiers, Hypothesis property tests, CI
gating, tagged releases, and OIDC-signed PyPI wheels --- into one
...
(RLB-v1), shown tight by a witness trace.
...
\texttt{pip install project-ghost==0.2.3}. \textbf{A safety claim
should be issued together with everything a third party needs to
reject it, and we believe that is now operationally possible.}
\end{abstract}
...
\begin{itemize}
  \item \textbf{C1 --- The executable safety citation pattern
        (conceptual).} A safety claim is published as a
        content-addressed run plus a pure-function verifier,
        distributable as one shippable unit a third party
        re-derives from a single command without trusting the
        producer. The cited artefact \emph{is} the falsification
        mechanism.
  \item \textbf{C2 --- Reference implementation: Ghost
        (artefact).} A closed-loop autonomy supervisor
        instantiating the pattern on five properties (BAUD-v1,
        ERUR-v1, MD-v1, RLB-v1, FPB-v1) with binding ADRs,
        content-addressed MCAP telemetry,
        \texttt{ghost verify-properties --mcap}, OIDC-signed PyPI
        wheels, and a policy-agnostic verifier across three
...
presented as a \textbf{supporting result} that \texttt{Rlb.tla}
mechanises, not as a contribution. We position the work as a
\textbf{systems / tools paper, not a theory paper}.
...
load-bearing contribution is operational, not theoretical: an
ADR, a content-addressed MCAP, a pure-function CLI verifier,
Hypothesis property tests, TLA+/TLC checks on the underlying
invariants, and an OIDC-signed PyPI wheel --- assembled so that
the citation \emph{is} the falsification mechanism. We call the
pattern an \textbf{executable safety citation}.

The reference instantiation on a five-property autonomy supervisor
is evidence that the pattern is workable, not the contribution
itself. What motivated the paper is that the \emph{assembly} makes
safety claims operationally falsifiable in a way that
prose-and-video assertions are not. The artifact is re-runnable
from \texttt{pip install project-ghost==0.2.3}; the contribution
stands or falls on whether the resulting verdict means what the
ADR says it means. We believe it does.
