...
\textbf{Project Ghost introduces executable safety citations: a
reproducibility pattern that allows a third party to verify a
safety claim against a content-addressed autonomy log using a
single command.} If a reader remembers one sentence from this
paper, that is the one we ask them to remember. The contributions
...
\begin{itemize}
  \item \textbf{C1 --- A safety citation pattern.} A composition of
        ...
        command, without trusting the producer. \textbf{Ghost turns
        safety claims into executable citations.} The pattern is what
        we believe is genuinely differentiating; the rest of the
        paper is the evidence that it works in practice, including on
        real flight telemetry (\S\ref{sec:realflight}).
...
Theorem~\ref{thm:rlb} --- the closed-form recovery latency bound
...
property. \textbf{The contribution of this paper is the assembly of
the seven boxes into one shippable unit, so that --- for the first
time, to the best of our knowledge within the surveyed venues --- a
safety claim can be issued together with everything a third party
needs to reject it.} The property set, the closed-form bound, and
the TLA+ specs instantiate the pattern on a representative
supervisor.}
\label{fig:pattern}
\end{figure}
...
\textbf{What this section does not claim.} It does not claim that
every class of bug in \S8.2 will flip on every real ULog --- that
requires a more diverse fixture set, scope for ADR-0037. It does
not claim that the buggy run is unsafe in a hardware sense --- the
buggy run did not fly anything. It claims, precisely, that the
verifier discriminates real telemetry against two specific named
regressions whose synthetic counterparts \S8.2 already detects.
