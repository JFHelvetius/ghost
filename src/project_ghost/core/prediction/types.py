...
def _validate_nonneg_vec3(arr: np.ndarray, *, field: str) -> None:
    ...
    if not np.all(np.isfinite(arr)):
        raise ValueError(
            f"{field} must be finite; got {arr!r}"
        )
    if np.any(arr < 0.0):
        raise ValueError(
            f"{field} must be >= 0; got {arr!r}"
        )
...
class PoseStd:
    ...
    def __post_init__(self) -> None:
        _validate_nonneg_vec3(
            self.position_std_enu_m, field="position_std_enu_m"
        )
        _validate_nonneg_vec3(
            self.orientation_std_rad, field="orientation_std_rad"
        )
...
class BeliefForwardPrediction:
    ...
    def __post_init__(self) -> None:
        if self.source_belief_stamp_sim_ns < 0:
            raise ValueError(
                f"source_belief_stamp_sim_ns must be >= 0; got "
                f"{self.source_belief_stamp_sim_ns}"
            )
        ...
        expected_stamp = (
            self.source_belief_stamp_sim_ns + self.horizon_ns
        )
        ...
        if not isinstance(self.predicted_pose, Pose):
            raise TypeError(
                f"predicted_pose must be Pose; got "
                f"{type(self.predicted_pose).__name__}"
            )
        if not isinstance(self.predicted_pose_std, PoseStd):
            raise TypeError(
                f"predicted_pose_std must be PoseStd; got "
                f"{type(self.predicted_pose_std).__name__}"
            )
        ...
        if self.schema_version != PREDICTION_PROTOCOL_VERSION:
            raise ValueError(
                f"schema_version must be {PREDICTION_PROTOCOL_VERSION}; "
